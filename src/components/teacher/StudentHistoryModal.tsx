  ...
  const renderSubmissionContent = (item: any) => {
    ...
    return (
      <div className="space-y-8">
        ...
        {item.image_link && (
          <div className="mb-4">
            <h4 className="text-[10px] font-bold text-muted-foreground uppercase mb-2 border-b pb-1">Attached Output</h4>
            <img
              src={item.image_link}
              alt="Student Output"
              className="max-h-[300px] w-auto border border-border rounded p-1 block"
        ...
        {Object.entries(docSections).map(([key, val]) => (
          <div key={key}>
            <h4 className="text-[10px] font-bold text-muted-foreground uppercase mb-2 border-b pb-1">{key}</h4>
            <div className="prose prose-sm max-w-none text-foreground" dangerouslySetInnerHTML={{ __html: val }} />
          </div>
        ))}
        ...
        {parsed.code && (
          <div>
            ...
            <pre className="bg-muted p-4 rounded border border-border text-[10px] font-mono whitespace-pre-wrap text-foreground leading-snug overflow-x-auto break-all">
              {parsed.code}
            </pre>
          </div>
        )}
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl h-[85vh] flex flex-col p-0">
          <DialogHeader className="px-6 py-4 border-b">
            <DialogTitle className="flex flex-col gap-1 pr-10">
                ...
                {loading ? (
                  ...
                  history.map((item) => (
                    <TableRow key={item.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        {item.status === 'evaluated' ? <Badge className="bg-green-500/10 text-green-600 dark:text-green-400 hover:bg-green-500/10 border-green-500/20">Graded</Badge> :
                          item.status === 'submitted' ? <Badge className="bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-500/10 border-blue-500/20">Submitted</Badge> :
                            item.status === 'redo_requested' ? <Badge variant="destructive" className="bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20">Redo</Badge> :
                      ...
                      <TableCell className="text-right">
                        {(item.status === 'submitted' || item.status === 'evaluated') && (
                          <Button size="sm" variant="ghost" onClick={() => setViewSubmission(item)} className="text-blue-600 hover:text-blue-700 hover:bg-blue-500/10">
      ...
      {viewSubmission && (
        <Dialog open={!!viewSubmission} onOpenChange={(val) => !val && setViewSubmission(null)}>
          <DialogContent className="max-w-[95vw] w-full h-[95vh] flex flex-col p-0 gap-0 overflow-hidden bg-muted">
            <DialogHeader className="px-6 py-3 border-b flex flex-row items-center justify-between bg-card shrink-0">
            ...
            <ScrollArea className="flex-1 bg-muted/50 p-8 flex justify-center">
              <div className="w-[210mm] min-h-[297mm] bg-white text-slate-900 shadow-xl p-[15mm] relative flex flex-col">
                ...
                {/* Info Grid */}
                <div className="grid grid-cols-2 gap-y-3 gap-x-8 text-sm mb-8 p-5 bg-muted/50 border border-border rounded-lg">
                  ...
                  <div className="flex justify-between border-b border-dashed border-border pb-1">
                    <span className="text-muted-foreground font-medium">Experiment</span>
                ...
                <div className="mt-auto pt-8 border-t border-slate-200 flex items-end justify-between text-xs text-slate-400">
                  ...
                  {viewSubmission.status === 'evaluated' && (
                    <div className="w-48 border border-border rounded bg-muted/50 p-2 text-left relative">
                      <div className="absolute -top-3 -right-3 bg-green-500/10 text-green-600 rounded-full p-1 border border-green-500/20 shadow-sm">
                        <Check size={16} strokeWidth={3} />
                      </div>
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Status</p>
                      <p className="text-sm font-bold text-foreground leading-tight">Graded & Verified</p>
                    </div>
                  )}
                </div>
              </div>
